...
The docker image will run matrix-voice-messenger with a SQLite database and
...
* Choose whether you want to use SQLite or Postgres as your database backend.
  ...
  ```
  database: "postgres://username:password@postgres/matrix-voice-messenger?sslmode=disable"
  ```
...
```
docker-compose up matrix-voice-messenger
```
...
```
docker-compose up -d matrix-voice-messenger
```

(Logs can later be accessed with the `docker logs` command).

This will use the `latest` tag from
[Docker Hub](https://hub.docker.com/somebody/matrix-voice-messenger).
...
```
docker-compose pull matrix-voice-messenger
```
...
[matrix-voice-messenger.service](matrix-voice-messenger.service). The service uses
`docker-compose` to start and stop the bot.

Copy the file to `/etc/systemd/system/matrix-voice-messenger.service` and edit to
match your setup. You can then start the bot with:

```
systemctl start matrix-voice-messenger
```

and stop it with:

```
systemctl stop matrix-voice-messenger
```

To run the bot on system startup:

```
systemctl enable matrix-voice-messenger
```
...
```
docker build -t somebody/matrix-voice-messenger:latest -f docker/Dockerfile .
```
